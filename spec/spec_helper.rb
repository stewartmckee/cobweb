...
RSpec.configure do |config|
  ...
  config.before(:each) {
    ...
    Net::HTTP::Get.stub!(:new).with("/redirect.html", {"User-Agent"=>"cobweb/1.0.1 (ruby/1.9.3 nokogiri/1.5.0)"}).and_return(@mock_http_redirect_request)
    Net::HTTP::Get.stub!(:new).with("/robots.txt", {"User-Agent"=>"cobweb/1.0.1 (ruby/1.9.3 nokogiri/1.5.0)"}).and_return(@mock_http_robot_request)
    Net::HTTP::Get.stub!(:new).with("/redirect2.html", {"User-Agent"=>"cobweb/1.0.1 (ruby/1.9.3 nokogiri/1.5.0)"}).and_return(@mock_http_redirect_request2)
